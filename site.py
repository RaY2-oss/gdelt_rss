...
def tail_over(s):
    ...
    лишних три мегабайта. Издание в паре пустое у всего, что пересказано, и
    заполнено у обзоров, отобранных дословно до 09.08.2026.
    ...
    сколько обошёлся сам обзор. Пусто у сюжета — это нормально: пересказа ещё
    не хватило бюджета или он не прошёл сторож. Подставлять на это место
    подводку статьи нельзя, она чужой текст (см. макрос digest).
    """
    out, total = [], 0
...
def stories(conn, rows, country, now):
    ...
    for key, (lab, weight, age) in zip(keys, ranked):
        ...
        # Обзор — пересказ всего сюжета, поэтому адрес у строки пустой: она не
        # принадлежит одному изданию и подписывать её одним изданием нельзя.
        # Пара с адресом осталась в формате ради строк, лежащих в кэше от
        # прежнего, дословного отбора: они доживут свои две недели подписанными.
        ...
        over = [{"text": s,
                 "domain": by_url.get(_safe_url(u), imp.domain(u)) if u else "",
                 "url": _safe_url(u) if u else ""} for u, s in lines
