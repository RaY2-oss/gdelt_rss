...
def _live(age_days):
    """Свежесть сюжета для боковой полосы: 1 — только что, 0 — край окна.

    Почему не importance.fade, которым строка выцветала раньше. fade — это доля
    веса, отнятая затуханием, и на живой ленте она почти вся лежит в 0.05–0.25:
    между вчерашним сюжетом и позавчерашним разница выходила в проценты, и
    показать её нечем — ни цветом, ни длиной. Ровно на это и была жалоба.

    Полоса меряет не вес, а возраст, и по логарифму часов, потому что новость
    стареет неравномерно: между «час назад» и «утром» разницы больше, чем между
    «неделю назад» и «десять дней назад». Шкала растянута на всё окно витрины,
    поэтому концы всегда заняты: сегодняшний сюжет горит целиком, сюжет с края
    архива — засечкой. Шесть часов в знаменателе — цена деления у верха: внутри
    первых шести часов полоса почти не меняется, дальше идёт вниз заметно.

    Ориентиры при окне в 14 суток: час — 0.96, полдня — 0.73, сутки — 0.60,
    трое — 0.37, неделя — 0.17.
    """
    hours = max(0.0, age_days) * 24.0
    span = math.log2(1.0 + SITE_HOURS / 6.0)
    return max(0.0, min(1.0, 1.0 - math.log2(1.0 + hours / 6.0) / span))


def tail_over(s):
    """Обзор сюжета для карточки из корпуса: [[строка, издание], ...].

    Тот же обзор, что на странице, но две строки вместо трёх и вдвое короче:
    search.json грузится целиком, а полный обзор на шесть тысяч сюжетов весит
    лишних три мегабайта.

    Издания, из которых сложен пересказ, едут сюда именами через запятую, а не
    ссылками: адрес втрое длиннее имени, а выход наружу у карточки хвоста один
    — издание, возглавившее сюжет. Вопрос «откуда это» имя закрывает, вопрос
    «дай почитать» — ссылка ниже.

    Пусто у сюжета — это нормально: пересказа ещё не хватило бюджета или он не
    прошёл сторож. Подставлять на это место подводку статьи нельзя, она чужой
    текст (см. макрос digest).
    """
    out, total = [], 0
    for o in s["overview"][:TAIL_OVER_LINES]:
        if out and total + len(o["text"]) > TAIL_OVER_CHARS:
            break
        out.append([o["text"], ", ".join(x["domain"] for x in o["srcs"])])
        total += len(o["text"])
    return out
...
def stories(conn, rows, country, now):
    ...
    for key, (lab, weight, age) in zip(keys, ranked):
        ...
        # предложения в него вошли (overview.make возвращает именно их). Список
        # источников сюжета ниже отвечает на вопрос «кто ещё об этом написал»,
        # а подпись под обзором — на другой: «откуда взято вот это».
        # Издания из restricted вычёркиваются из подписи, а не отменяют её:
        # запасной imp.domain(u) как раз и подставил бы вырезанное имя.
        over = []
        for us, s in lines:
            if not s:
                continue
            us = [us] if isinstance(us, str) else list(us)
            srcs = [{"url": _safe_url(u), "domain": by_url.get(_safe_url(u), imp.domain(u))}
                    for u in us if u and not restricted.is_restricted(imp.domain(u))]
            over.append({"text": s, "srcs": srcs})
        ...
        out.append({
            ...
            # Обзор по всем изданиям сюжета. Пусто — значит пересказа пока нет,
            # и тогда у сюжета остаётся только заголовок: подставлять подводку
            # статьи нельзя, это чужой текст.
            "overview": over,
            "updated": updated,
            ...
            # Свежесть для боковой полосы. Три знака — значение уходит в
            # атрибут style= на каждой из шести тысяч строк, а дальше третьего
            # разницы в длине полосы не видно.
            "live": round(_live(age), 3),
            "iso": dt.isoformat(),
            ...
            # нашлось хотя бы у одного. Телом — обзор, а пока его нет, текст
            # статьи: наружу это не идёт, здесь он только сырьё классификатора.
    ...
    # Фиксация остатка по стране: переводы представителей и всё, что не успел
    # зафиксировать сам overview.resolve (тот фиксирует каждый пересказ сразу,
    # иначе транзакция висела бы всю страну и роняла сбор с переводчиком).
    if conn is not None:
        conn.commit()
...
def search_index(everything, regions, near=None):
    ...
    return {
        ...
        "s": [[s["title"], ci[s["country"]], di[s["day"]], s["score"],
               ...
               tail_over(s), s["outlets"], s["tp"], s["live"]]
              for s in everything],
...
def build(out_dir=OUT_DIR):
    ...
    try:
        ...
        # Бюджет пересказов на сборку. Без этого вызова обзоры только читаются
        # из кэша: пересказ стоит секунды на сюжет, и сборка без потолка шла бы
        # сутки. Бюджет один на все страны и тратится в их порядке, поэтому
        # первые дни текст появляется сверху списка стран, а не по важности;
        # это само проходит — за сутки сборки успевают больше сюжетов, чем
        # приходит новых, так что кэш догоняет ленту и дальше идёт вровень.
        overview.begin()
...
def _selfcheck():
    ...
    # Свежесть: концы шкалы заняты, середина растянута. Порядок важнее точных
    # чисел — полоса обязана убывать монотонно и заметно в первые сутки.
    assert _live(0) == 1.0 and _live(SITE_DAYS) == 0.0
    assert _live(-1) == 1.0 and _live(SITE_DAYS * 3) == 0.0
    assert _live(1 / 24) > _live(0.5) > _live(1) > _live(3) > _live(7)
    assert _live(0.5) - _live(1) > 0.1, "сутки от полусуток должны отличаться"
    ...
    # Чужой абзац на витрину не попадает ни при каких условиях: подводки статьи
    # больше нет как поля, а пересказ без открытого бюджета (begin) не делается.
    assert "snippet" not in s and not s["overview"], s.keys()
    ...
    # Пересказ подписан изданиями, из которых сложен (overview.make отдаёт их
    # списком). В хвост поиска они едут именами через запятую, без ссылок.
    assert tail_over({"overview": [
        {"text": "Пересказ сюжета.",
         "srcs": [{"domain": "a.com", "url": "https://a.com/1"},
                  {"domain": "b.com", "url": "https://b.com/2"}]}]}) == [
        ["Пересказ сюжета.", "a.com, b.com"]]
