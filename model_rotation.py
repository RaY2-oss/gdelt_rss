...
# сразу, а провалившийся пул отдаёт ход следующему провайдеру в кольце.
...
# Кулдауна провайдера здесь нет намеренно: провайдер, у которого весь пул
# провалился, из кольца НЕ выбывает — следующий вызов пробует его снова. 429
# бывает и частотным (отпускает за десятки секунд), и суточным, отличить их по
# ответу нельзя, а фиксированные 600с выбивали живого провайдера из ротации на
# весь прогон. Провал пула стоит один обход моделей, а не остаток прогона.
...
# выбывают. За один вызов делаем не более одного круга.
# ---------------------------------------------------------------------------
