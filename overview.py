...
Потом пересказ (`make`): отобранные предложения уходят в общий пул моделей
(model_rotation, тот же, что судит статьи), и наружу идёт только его ответ.

Почему пул, а не своя модель на машине
--------------------------------------
До 09.08.2026 пересказывала rut5_base_sum_gazeta (222 млн) в CTranslate2 —
своя, бесплатная, без сети. Она врала, и не по мелочи. Замер на десяти
настоящих сюжетах из базы: в сюжете про клинику Майо «препарат разработали
против рака» превратилось в «препарат помогает раку расти и сопротивляться
лечению»; министру ИТ Телинганы модель выдала должность министра иностранных
дел Индии; в сюжете про китайские вузы сокращения гуманитарных программ
переехали на ИИ и робототехнику, то есть ровно наоборот. Три вранья из
десяти, причём такого, которое читатель принимает за факт.

Ловить это на месте нечем. Проверялись две NLI-модели (rubert-base-nli и
mDeBERTa-xnli, схема SummaC): на выдуманных примерах они врут узнают, а на
этих трёх настоящих — нет, зато отбраковывают верные фразы, и стоят 11-24 с
на сюжет. Отдельная модель-проверяльщик обошлась бы дороже пересказа и
всё равно не дала бы обещания «не врёт».

Пул на тех же десяти сюжетах не соврал ни разу, вдвое быстрее (9 с против
19 с) и ничего не занимает на диске. Цена — суточная квота, общая с судьёй
статей, поэтому бюджет сборки (`begin`, MAKE_SECONDS) теперь меряет не только
время, но и расход квоты, а серия отказов пула закрывает бюджет досрочно.

Чем пул всё-таки врёт
---------------------
Не смыслом, а единицами. Замер по 58 живым обзорам 09.08.2026: должностей и
отрицаний пул не переворачивал ни разу, зато индийский тариф в 5 рупий вышел
тарифом «5 руб.», субсидия в 2 крор рупий — «2 крон рупий», охват в 71 лакх
домохозяйств — «71 л. домохозяйств». Числа при этом сходятся с источником, и
сторож чисел такое пропускает, а читатель принимает за факт.

Ловится это ровно тем же способом, что и числа: единица счёта из закрытого
списка, которой в источнике нет, — подмена (`_money`). Единицы есть у десяти
обзоров из 58, отбраковались эти три, годных не задето. Морфологию берёт
основа: «рупий» и «рупия» — одно, «крор» и «крон» — разное.

Чем это смягчено дополнительно: на вход идёт не статья, а уже отобранные
центральные предложения — врать не по чему, кроме сказанного всеми
источниками сразу; не прошедшее сторож (`guarded`) в кэш не пишется, и сюжет
остаётся вовсе без обзора. Пустое место лучше уверенного вранья.
...
# Пересказ (`make`). SRC_CHARS — сколько текста уходит в модель: больше
# отобранных предложений на входе, чем показывает витрина, но не вся статья.
SRC_CHARS = int(os.environ.get("OVERVIEW_SRC_CHARS", "1700"))
MIN_OUT_CHARS = 80
MAX_OUT_CHARS = 600       # промпт просит 300; всё, что вдвое длиннее, — не пересказ
REPEAT_MIN = 0.62         # доля разных слов в пересказе, ниже — зацикливание
# Сколько слов подряд пересказ вправе повторить из источника. Это не борьба с
# ленью модели, а обещание витрины: чужого текста на ней нет даже в переводе
# (/legal.html).
#
# Двенадцать — замер по тридцати живым пересказам, а не круглое число. Восемь,
# поставленные на глаз, отбраковали треть годных: длинные совпадения дают не
# списанные фразы, а перечисления — «более 2000 ледников и 3219 ледниковых
# озёр в Ладакхе», «с 3 ГВт в 2014 году до 162,15 ГВт». Имена, числа и
# названия ведомств в пересказе обязаны стоять теми же словами, иначе он
# соврёт, и на них набегает 9-11 слов подряд без всякого списывания. Настоящее
...
# сделать и не нужно: resolve идёт по убыванию важности, поэтому бюджет
# тратится сверху вниз, и верх ленты обзаводится текстом в первую же сборку, а
# хвост архива — за несколько суток. Кэш живёт две недели, сделанное не
# пропадает.
#
# Пять минут — это примерно 30 сюжетов в час при новых 600 в сутки, то есть
# новое успевает пересказаться, а квоты судье остаётся вдоволь (он тратит
# десятки вызовов в сутки, не тысячи). Ставить больше — отбирать у судьи.
MAKE_SECONDS = float(os.environ.get("OVERVIEW_MAKE_SECONDS", "300"))
# Подряд идущих отказов пула, после которых сборка перестаёт спрашивать. Пул
# отказывает всем сразу — обычно это исчерпанная суточная квота, и следующие
# двести попыток кончатся так же, только медленнее.
FAIL_STOP = 3
_until = None
_fails = 0
...
def begin(seconds=None):
    ...
    global _until, _fails
    _until = time.monotonic() + (MAKE_SECONDS if seconds is None else seconds)
    _fails = 0


def stop():
    """Закрыть бюджет досрочно: спрашивать больше некого."""
    global _until
    _until = None
...
_NUM = re.compile(r"\d+")
# Кириллица и латиница внутри одного слова (см. guarded).
_MIXED = re.compile(r"[а-яё][a-z]|[a-z][а-яё]", re.I)
# Валюта и индийские единицы счёта (см. guarded). Хвост ограничен тремя
# буквами — это окончание, а не продолжение слова: иначе «Ленина» дала бы
# «л», а «лет» перекрыло бы подмену лакха.
_MONEY = re.compile(
    r"(?<![а-яё])(руб|рупи|крор|лакх|крон|доллар|евро|фунт|юан|иен)[а-яё]{0,3}(?![а-яё])"
    r"|\d\s*(л)\.", re.I)
...
def _tidy(text):
    ...
    if not t:
        return ""
    # Неразрывный дефис (U+2011): модель ставит его в «Премьер‑министр» и
    # «2025‑26» на ровном месте — в источнике всюду обычный. Читателю разницы
    # нет, а поиску по витрине и сверке с источником есть.
    t = t.replace("‑", "-")
    if t[-1] not in ".!?…»\"":
...
def guarded(summary, source):
    ...
    Ловит выдуманные суммы, даты и количества, подменённую валюту, отказ
    модели, её же рассуждения вслух и дословное списывание. Смысловое враньё не
    ловит и не притворяется, что ловит: за это отвечает модель на том конце и
    отбор на входе. Отказ — штатный исход, а не ошибка.
    """
    t = _tidy(summary)
    if not t:
        return None
    # Пятое правило промпта: в кластере не оказалось новости. Отказ, а не текст.
    if re.fullmatch(r"[«\"']?нет[.!»\"']*", t, re.I):
        return None
    # Reasoning-модель, у которой рассуждение не закрылось тегом. Ответа в нём
    # нет — есть черновик мысли; model_rotation такое режет, но проверить у
    # себя дешевле, чем однажды показать это читателю.
    if "<think" in t.lower():
        log.info("overview: отбракован — думалка вместо ответа")
        return None
    if len(t) < MIN_OUT_CHARS or len(t) > MAX_OUT_CHARS:
        return None
    # Слово, склеенное из двух алфавитов: «полovina», «Индiя». Так у моделей
    # ломается декодирование на редком токене, и заметно это только глазами —
    # ни один смысловой сторож такого не видит, а читатель видит сразу.
    if _MIXED.search(t):
        log.info("overview: отбракован — слово из двух алфавитов")
        return None
    ...
    for n in _NUM.findall(t):
        if n not in have:
            ...
            return None
    # Единица при числе, которой в источнике нет. Сторож чисел это пропускает —
    # цифры-то сходятся, — а читает это как факт: индийский тариф «5 руб.»
    # вместо рупий, субсидия «2 крон рупий» вместо крор, «71 л. солнечных
    # систем» вместо лакха. Замер по 58 живым обзорам: единицы счёта есть у
    # десяти, отбраковываются ровно эти три, годных не задето.
    lost = _money(t) - _money(source)
    if lost:
        log.info("overview: отбракован — единица %s не из источника",
                 ", ".join(sorted(lost)))
        return None
    if t in source:
        return None           # дословная копия источника пересказом не является
    if _copied(t, source):
        log.info("overview: отбракован — списан дословно")
        return None
    return t


def _money(text):
    """Валюты и единицы счёта текста, приведённые к основе: {«рупи», «крор»}."""
    return {(m.group(1) or m.group(2)).lower() for m in _MONEY.finditer(text)}


def _copied(text, source):
    """Есть ли в пересказе кусок источника длиной COPY_RUN слов подряд."""
    w = _words(text)
    if len(w) < COPY_RUN:
        return False
    runs = _shingles(source, COPY_RUN)
...
def make(docs):
    ...
    ими. Остальные источники сюжета стоят на витрине отдельным списком.
    """
    global _fails
...
def resolve(conn, cached, key, urls, docs):
    ...
    предложениям сюжета, а не по всему, что попало в статьи.
...
def _selfcheck():
    ...
    # Отказ модели по пятому правилу промпта — не текст обзора.
    assert guarded("НЕТ", src) is None and guarded("Нет.", src) is None
    # Рассуждение вслух вместо ответа: закрывающего тега нет, ответа тоже.
    assert guarded("<think>Так, тут говорится про вложения компании, значит "
                   "надо написать про сто миллионов и про год", src) is None
    # Списанное предложение: двенадцать слов подряд из источника — уже не
    # пересказ, а перепечатка.
    long_src = ("Правительство штата санкционировало тридцать пять крор рупий "
                "лесному департаменту на посадку пальм вдоль сельских полей "
                "ради снижения числа смертей от ударов молнии.")
    assert guarded("Правительство штата санкционировало тридцать пять крор "
                   "рупий лесному департаменту на посадку пальм вдоль полей, "
                   "сообщил министр на совещании в четверг.", long_src) is None
    # А пересказ теми же словами, но своим порядком, проходит: имена и числа
    # обязаны совпадать, иначе пересказ соврёт.
    assert guarded("Лесной департамент получит тридцать пять крор рупий, чтобы "
                   "высадить пальмы вдоль сельских полей и снизить число "
                   "смертей от ударов молнии.", long_src)
    # Слово из двух алфавитов — след сломанного декодирования.
    assert guarded(ok.replace("производство", "проizводство"), src) is None

    # Подменённая единица счёта: числа сходятся, а факт другой. Все три —
    # настоящие пересказы из базы за 09.08.2026.
    money_src = ("Закупка SECI показала тариф 5 рупий за киловатт-час, а "
                 "схема предусматривает субсидию до 2 крор рупий за тонну "
                 "мощности и охват 71 лакха домохозяйств по стране.")
    assert guarded("Закупка SECI показала тариф 5 руб. за киловатт-час при "
                   "субсидии до 2 крор рупий за тонну мощности.", money_src) is None
    assert guarded("Схема предусматривает субсидию до 2 крон рупий за тонну "
                   "установленной мощности биогаза в стране.", money_src) is None
    assert guarded("Государственная схема обеспечила 71 л. домохозяйств "
                   "страны и субсидию до 2 крор рупий за тонну мощности.",
                   money_src) is None
    # Те же единицы, названные как в источнике, проходят — вместе со склонением.
    assert guarded("Тариф закупки SECI составил 5 рупий за киловатт-час, а "
                   "субсидия дошла до 2 крор рупий за тонну мощности.", money_src)
    # Слово, которое лишь начинается на «л», единицей счёта не считается —
    # иначе «за пять лет» перекрывало бы подмену лакха.
    assert not _money("Проект окупится за пять лет в Ленинградской области")

    # Неразрывный дефис модель ставит от себя; в источнике всюду обычный.
    assert guarded(ok.replace("100 млн", "100‑млн"), src) == \
        ok.replace("100 млн", "100-млн")
