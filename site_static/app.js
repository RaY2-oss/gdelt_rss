...
(function () {
  ...
  if (find && findBtn) {
    ...
    var render = function () {
      ...
      rows.slice(0, LIMIT).forEach(function (s, i) {
        ...
        li.style.setProperty('--live', s[10] == null ? 1 : s[10]);
  ...
  if (stories.length && feedBox) {
    ...
    var card = function (t, no) {
      ...
      // Свежесть приезжает из корпуса готовой (см. site.corpus): считать её
      // здесь по дате нельзя — в корпусе лежит день ПЕРВОЙ статьи сюжета, а
      // свежесть идёт от возраста последней. Ею css красит боковую полосу.
      art.style.setProperty('--live', s[10] == null ? 1 : s[10]);
      ...
      // Обзор — той же вёрсткой, что и в шаблоне (_story.html): пересказ и
      // подпись, из каких изданий он сложен. Подпись без ссылок: в корпусе
      // лежат только имена изданий (см. tail_over), а выход наружу у карточки
      // один, ниже.
      if (s[7] && s[7].length) {
        var over = el('div', 'over');
        s[7].forEach(function (o) {
          var line = el('p', 'over__line', o[0] + ' ');
          if (o[1]) line.appendChild(el('span', 'over__from', 'по данным ' + o[1]));
