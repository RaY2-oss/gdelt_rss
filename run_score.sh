...
# Прогон длиннее часа не должен встречаться со следующим: две стадии оценки
# разом — это двойной расход лимитов LLM и ~2 ГБ RSS на копию, а машина
# четырёхъядерная. Не взяли блокировку — тихо уходим, cron вернётся через час.
exec 9>"$BASE/.score.lock"
flock -n 9 || { echo "run_score.sh: предыдущий прогон ещё идёт — пропускаю" >&2; exit 0; }
