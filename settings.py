...
RELEVANT_SCORE = 100       # articles.importance у принятой статьи. Колонка осталась
                            # ДВОИЧНОЙ: LLM отвечает да/нет (см. pipeline._score_prompt),
                            # ранжирование целиком на структурной важности
                            # (importance.structural). Прежний порог MIN_IMPORTANCE=40
                            # снят вместе со шкалой 0..100 — отбирать по квантованной
                            # оценке было нечем, 96 % значений были кратны пяти.
DEDUP_COSINE = 0.95        # e5-косинус ЗАГОЛОВКОВ: выше — один сюжет, мержим безусловно
DEDUP_BODY_COSINE = 0.91   # то же для эмбеддингов ТЕЛ — порог НИЖЕ заголовочного.
                            # Тела разделяют сюжеты лучше заголовков: внутри сюжета
                            # p10 = 0.870 против p90 = 0.851 между разными сюжетами,
                            # тогда как у заголовков эти границы перехлёстываются
                            # (0.843 против 0.864) и порог там приходится держать
                            # высоко. Прежний общий 0.95 фактически глушил канал тел
                            # целиком: p99 косинусов тел — 0.874, до порога доходил
                            # меньше процента пар. Из-за этого ядерная сделка
                            # США — Саудовская Аравия (28.07) разъехалась на 7 позиций
                            # фида при косинусах тел 0.816..0.939; на 0.91 остаётся 4
                            # без единого ложного слияния (на 0.90 их уже два).
...
PREFILTER_TARGET_RECALL = 0.95   # какую долю нужных статей обязан сохранить.
                                 # Было 0.97, но цифры под ним считались по
                                 # протекавшей кросс-валидации (см. cv в
                                 # train_prefilter). На честных фолдах 0.97 режет
                                 # лишь 27 % мусора — ниже порога окупаемости
                                 # ниже; 0.95 режет 40.7 %. Цена перехода на той
                                 # же выборке 27 195: теряем 300 нужных статей
                                 # вместо 180, то есть +2 % к недобору ради
                                 # -14 п.п. обращений к LLM. Знак обратно —
                                 # ровно эта константа, переобучить и всё.
...
# Какую importance выставить принятому локально — ту же, что у принятого LLM:
# решение у них одно и то же, «по теме или нет». Порядок задаёт структурная
# важность (importance.structural), к этому значению он не привязан.
PREFILTER_ACCEPT_SCORE = RELEVANT_SCORE
...
# Граница "по теме / не по теме". Вердикт теперь двоичный (0 или
# RELEVANT_SCORE), так что порог — просто «не ноль»; отдельной константой он
# остался, потому что по нему же вердикт разбивается на accepted/rejected при
# записи в seen_urls, откуда его читает train_prefilter.py, и потому что в БД
# лежат старые статьи с оценками по прежней шкале 0..100.
RELEVANCE_CUTOFF = 5
