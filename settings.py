...
VERDICT_REUSE_HOURS = 24 * 8  # как далеко назад pipeline._reuse_known_verdicts ищет
                            # уже вынесенный вердикт для того же сюжета. Раньше брал
                            # KEEP_HOURS — совпадение, а не связь: глубина архива
                            # выросла до двух недель, а «то же событие всё ещё
                            # приезжает» осталось про дни. Копировать вердикт
                            # двухнедельной давности значит ещё и растянуть на две
                            # недели любую правку промпта: старое «да» военной
                            # статье переписывало бы новое «нет».
...
# camoufox — это целый Firefox: ~1.3 ГБ на диске и десяток секунд на страницу,
# поэтому он не участвует в общей загрузке, а достаётся только тем URL, кому
# эта попытка последняя (см. seen_store.SHORT_TRIES). Замер на двенадцати
# «трудных» адресах: браузер вытащил три, и все три — из тех, что отдавали 403.
# Пустые двухсотки (пейволл, текст рисуется скриптом) он не берёт тоже.
CAMOUFOX_MAX = 25
...
# ── MMR-диверсити (feeds._mmr_select) ────────────────────────────────────────
# Баланс важность/диверсити при отборе сюжетов в фид (0..1). Имя с приставкой
# DIGEST_ осталось от недельного дайджеста, который отсюда и брал параметр;
# сам дайджест снят 07.08.2026, а величина используется фидом и переименования
# не заслуживает — она в одном месте и подписана.
DIGEST_MMR_LAMBDA = 0.5
...
# Затухание важности со временем (importance.freshness). Витрина держит архив
# за две недели, и без затухания сюжет, набравший сорок изданий в понедельник,
# стоял бы над всем до конца окна. Половина веса за четверо суток: за 24 часа —
# окно фида и дайджеста — теряется меньше десятой, то есть там порядок почти не
# меняется, а двухнедельный хвост витрины оседает. Пол не даёт затуханию
# превратиться в удаление. 0 в HALF_LIFE выключает затухание.
IMPORTANCE_HALF_LIFE_DAYS = 4.0
IMPORTANCE_AGE_FLOOR = 0.35

# Глубина архива: сколько дней статья живёт в БД и показывается на витрине.
# Держится ОТДЕЛЬНО от окон ранжирования (IMPORTANCE_HALF_LIFE_DAYS, суточный
# фид): те про то, что важно сейчас, а архив — про то, за сколько дней читатель
# может пролистать ленту назад, и одно другому не указ.
ARCHIVE_DAYS = 14
KEEP_HOURS = 24 * (ARCHIVE_DAYS + 1)  # +сутки запаса: чистка идёт по часам,
                                      # и статья не должна исчезнуть с витрины
                                      # раньше, чем закончится её 14-й день.
